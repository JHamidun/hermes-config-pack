...
2. **Para cada URL pendiente**:
   ...
   b. **Extraer JD** usando Playwright (browser_navigate + browser_snapshot) → web_extract → web_search
...
2. **web_extract (fallback):** Para páginas estáticas o cuando Playwright no está disponible.
3. **web_search (último recurso):** Buscar en portales secundarios que indexan el JD.

**Casos especiales:**
- **LinkedIn**: Puede requerir login → marcar `[!]` y pedir al usuario que pegue el texto
- **PDF**: Si la URL apunta a un PDF, leerlo directamente con read_file tool
