...
// Data lives OUTSIDE the skill, in ~/career-ops (see SKILL.md -> Working Directory).
// Override with CAREER_OPS_HOME. Falls back to the script's own folder for the
// upstream single-directory layout, where tracker and scripts sit side by side.
// NB: fileURLToPath, not URL.pathname -- the latter yields "/C:/..." on Windows.
const SKILL_DIR = dirname(fileURLToPath(import.meta.url));
const CAREER_OPS = process.env.CAREER_OPS_HOME
  || (existsSync(join(homedir(), 'career-ops')) ? join(homedir(), 'career-ops') : SKILL_DIR);
...
// states.yml ships with the skill (templates/), but a user copy in the data dir wins.
const STATES_FILE = [
  join(CAREER_OPS, 'config/states.yml'),
  join(CAREER_OPS, 'states.yml'),
  join(SKILL_DIR, 'templates/states.yml'),
].find(existsSync) || join(SKILL_DIR, 'templates/states.yml');
