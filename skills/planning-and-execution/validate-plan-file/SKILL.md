...
description: "Проверка plan-файлов оркестратора по JSON-схеме до чтения."
user_description: "Служебная проверка: сверяет файл плана, который оркестратор раздаёт рабочим агентам, с ожидаемой структурой и точно указывает, какое поле не на месте. Нужен, когда многошаговый сценарий валится на середине и надо понять, виноват план или исполнение."
version: 1.0.0
license: MIT
metadata:
  hermes:
    category: planning-and-execution
    tags: [validate, plan, file, sql, claude]
    source: claude-code-config-pack
    requires_tools: [read_file]
---
## Когда применять

Проверка plan-файлов оркестратора по JSON-схеме до чтения воркерами. Служебный. Триггеры: «валидируй plan.json», «проверь план-файл».
...
Use read_file tool to load the plan file.
...
Use read_file tool to load the appropriate schema file from `.claude/schemas/`.
