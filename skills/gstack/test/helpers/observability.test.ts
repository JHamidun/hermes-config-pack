...
describe('eval-store observability', () => {
  ...
  test('6b: savePartial() accumulates multiple tests', () => {
    ...
    collector.addTest({
      ...
      exit_reason: 'timeout', timeout_at_turn: 5, last_tool_call: 'terminal(ls)',
    });
    ...
    expect(partial.tests[1].last_tool_call).toBe('terminal(ls)');
  });
  ...
  test('EvalTestEntry includes diagnostic fields', () => {
    ...
    collector.addTest({
      ...
      last_tool_call: 'write_file(review-output.md)',
    });
    ...
    expect(t.last_tool_call).toBe('write_file(review-output.md)');
  });
});
...
describe('eval-watch dashboard', () => {
  test('9: renderDashboard shows completed tests and current test', () => {
    const heartbeat: HeartbeatData = {
      ...
      turn: 4,
      toolCount: 3,
      lastTool: 'write_file(review-output.md)',
    ...
    expect(output).toContain('write_file(review-output.md)');
