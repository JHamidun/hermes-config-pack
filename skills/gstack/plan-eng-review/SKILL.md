...
metadata:
  hermes:
    ...
    tags: [gstack, plan, eng, review, git, claude]
...
_GS="~/.hermes/skills/browser-and-automation/gstack"; [ -f "$_GS/bin/gstack-update-check" ] || _GS=".claude/skills/gstack"
...
If output shows `UPGRADE_AVAILABLE <old> <new>`: read `~/.hermes/skills/browser-and-automation/gstack/gstack-upgrade/SKILL.md` and follow the "Inline upgrade flow" (auto-upgrade if configured, otherwise clarify with 4 options, write snooze state if declined). If `JUST_UPGRADED <from> <to>`: tell user "Running gstack v{to} (just updated!)" and continue. If `UPDATE_CHECK_UNAVAILABLE <ver> <reason>` or `UPDATE_CHECK_BROKEN …`: the version check could not run — say so in one line and continue; do not treat it as up to date.

## clarify Format

**ALWAYS follow this structure for every clarify call:**
...
3. **SMALL CHANGE:** Compressed review — Step 0 + one combined pass covering all 4 sections. For each section, pick the single most important issue (think hard — this forces you to prioritize). Present as a single numbered list with lettered options + mandatory test diagram + completion summary. One clarify round at the end. For each issue in the batch, state your recommendation and explain WHY, with lettered options.
...
**STOP.** For each issue found in this section, call clarify individually. One issue per call. Present options, state your recommendation, explain WHY. Do NOT batch multiple issues into one clarify. Only proceed to the next section after ALL issues in this section are resolved.
...
For LLM/prompt changes: check the "Prompt/LLM changes" file patterns listed in CLAUDE.md. If this plan touches ANY of those patterns, state which eval suites must be run, which cases should be added, and what baselines to compare against. Then use clarify to confirm the eval scope with the user.
...
Follow the clarify format from the Preamble above. Additional rules for plan reviews:
* **One issue = one clarify call.** Never combine multiple issues into one question.
...
* **Escape hatch:** If a section has no issues, say so and move on. If an issue has an obvious fix with no real alternatives, state what you'll do and move on — don't waste a question on it. Only use clarify when there is a genuine decision with meaningful tradeoffs.
* **Exception:** SMALL CHANGE mode intentionally batches one issue per section into a single clarify at the end — but each issue in that batch still requires its own recommendation + WHY + lettered options.
...
### TODOS.md updates
After all review sections are complete, present each potential TODO as its own individual clarify. Never batch TODOs — one per question. Never silently skip this step. Follow the format in `.claude/skills/review/TODOS-format.md`.
...
If the user does not respond to an clarify or interrupts to move on, note which decisions were left unresolved. At the end of the review, list these as "Unresolved decisions that may bite you later" — never silently default to an option.
