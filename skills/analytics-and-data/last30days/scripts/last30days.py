...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--auto-resolve", action="store_true",
                        help="Use web search to discover subreddits/handles before planning (for platforms without web_search)")
...
def _show_runtime_ui(
    ...
    # Hermes, Gemini) is driving — those already have web_search and can
...
def _main(
    ...
    try:
        ...
        # without web_search (OpenClaw, Codex, raw CLI).
            ...
            if comp_explicit:
                ...
                if not resolve_mod._has_backend(config) and not args.mock:
                    sys.stderr.write(
                        ...
                        "Hermes, Gemini, any agent with a web_search tool): YOU have "
                        "web_search. Use it to run full Step 0.55 per entity, then invoke "
                        "the engine with a vs-topic plus --competitors-plan:\n"
                        "  1. web_search for '{topic} competitors' or '{topic} alternatives'.\n"
                        "  2. For each peer, web_search for handles/subs/github (Step 0.55).\n"
