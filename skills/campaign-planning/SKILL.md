...
description: "Собирает план маркетинговой кампании."
user_description: "Собирает план маркетинговой кампании: цель, сегмент аудитории, сообщение, каналы, бюджет и дерево метрик от показа до стоимости клиента. Нужен, когда кампанию надо не просто запустить, а потом честно оценить — сработала она или нет."
version: 1.0.0
license: MIT
metadata:
  hermes:
    category: marketing
    tags: [campaign, planning, node, video]
    source: claude-code-config-pack
---
## Когда применять

Планирование маркетинговых кампаний: цели, сегменты, каналы, KPI, бюджет, дерево метрик CPM→CTR→CPL→CPA→LTV→CAC. Триггеры: «маркетинговый план».

# Campaign Planning Skill

Frameworks and guidance for planning, structuring, and executing marketing campaigns.

**Read `~/.hermes/ccpack/business-context.md` first** — sections "Product", "ICP", "Funnel" (current step values), "Channels" (what already works and on what budget) and "Economics" (what a lead and a customer cost you). No such file? Create it from `~/.hermes/ccpack/templates/business-context.md`.
