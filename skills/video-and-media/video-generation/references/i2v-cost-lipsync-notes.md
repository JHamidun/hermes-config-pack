...
| Route | Cost | Verdict |
|---|---|---|
| **Veo 3.1 Fast via Google API** (`veo-3.1-fast-generate-preview`, `GOOGLE_API_KEY`) | Google budget, ~44-54s/6s-clip | **DEFAULT for i2v motion.** Not Runway credits. Script pattern: `_kamila_birthday/scripts/veo_animate.py` (JPG keyframe → mime `image/jpeg`, aspect 9:16, `duration_seconds`∈{4,6,8}, workers≤3, soften-retry on safety filter). Veo emits native audio → strip with `-an` at assembly. |
