---
name: video-editor
description: "Видеомонтаж FFmpeg+Python: тишина, субтитры, рефрейм 9:16."
version: 1.0.0
license: MIT
metadata:
  hermes:
    category: video-and-media
    tags: [video, editor, ffmpeg, python, git, gemini, audio]
    source: claude-code-config-pack
---
## Когда применять

Видеомонтаж FFmpeg+Python: тишина, субтитры, рефрейм 9:16. Триггеры: «вырежи паузы», «склей видео», «наложи музыку». НЕ AI-генерация→video-generation.
...
```bash
python ~/.hermes/skills/video-and-media/video-editor/video_editor.py <command> [args]
```
...
```bash
python ~/.hermes/skills/video-and-media/video-editor/video_editor.py process \
  input.mp4 \
  --music \
  --brightness 1.1 \
  --contrast 1.2 \
...
```bash
python ~/.hermes/skills/video-and-media/video-editor/video_editor.py trim \
  input.mp4 \
  --start 00:00:10 \
  --end 00:01:30 \
  -o clip.mp4
...
```bash
python ~/.hermes/skills/video-and-media/video-editor/video_editor.py probe input.mp4
```
