...
0.55/0.75 which use Claude Code's web_search tool.
"""
...
def _merge_category_peers(topic: str, subreddits: list[str]) -> tuple[list[str], Optional[str]]:
    """Extend the web_search-extracted subreddit list with category peers.
    ...
    every web_search-returned sub (they are the freshest signal) and
    ...
    actually added (not when every peer was already in the web_search set).
