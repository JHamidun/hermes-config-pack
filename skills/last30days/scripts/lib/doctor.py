...
def _sc_fix() -> str:
    return _fix_text(prescriptions.get("scraper-vendor", "key_missing"))


def _sc_gated_record(config: Dict[str, Any], purpose: str) -> Dict[str, Any]:
    if config.get("SCRAPER_API_KEY"):
        return _record(status=health.OK, requires="SCRAPER_API_KEY",
                       detail=f"SCRAPER_API_KEY present ({purpose})")
    return _record(status="unconfigured", requires="SCRAPER_API_KEY",
                   fix=_sc_fix())
...
def _sc_optin_record(config: Dict[str, Any], source: str, purpose: str) -> Dict[str, Any]:
    ...
    """
    requires = f"SCRAPER_API_KEY + INCLUDE_SOURCES={source}"
...
def _youtube_record(config):
    ...
    # is absent and the legacy ScraperVendor path is the only one left. Never
    # prescribe a paid key for something the installed toolchain already does.
    if not env.is_youtube_comments_available(config):
        notes.append(
            "comment text needs yt-dlp (free) or a ScraperVendor key "
...
def _linkedin_record(config):
    requires = "SCRAPER_API_KEY + INCLUDE_SOURCES=linkedin"
    if not config.get("SCRAPER_API_KEY"):
        return _record(status="unconfigured", requires=requires, fix=_sc_fix())
...
def _pinterest_record(config):
    requires = "SCRAPER_API_KEY; requested-only (--search pinterest)"
    if not config.get("SCRAPER_API_KEY"):
        return _record(status="unconfigured", requires=requires, fix=_sc_fix())
    return _record(
        status="opt-in", requires=requires,
...
def _sub_lanes_for(source: str, config: Dict[str, Any]):
    ...
    has_sc = bool(config.get("SCRAPER_API_KEY"))
    if source == "reddit":
        backups.append({
            "name": "ScraperVendor backfill", "armed": has_sc,
            "note": "fills in when the free public path returns nothing",
        })
    elif source == "youtube":
        backups.append({
            "name": "ScraperVendor transcript/search backstop", "armed": has_sc,
...
# live-probed - a health check must never spend ScraperVendor credits or trip
