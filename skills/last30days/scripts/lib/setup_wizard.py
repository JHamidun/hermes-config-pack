...
Want TikTok and Instagram too? ScraperVendor adds those (10,000 free calls, your-scraper.example). No kickbacks, no affiliation.
...
def write_api_key(env_path: Path, api_key: str, key_name: str = "SCRAPER_API_KEY") -> bool:
    ...
    Args:
        ...
        key_name: The env var name to write (default SCRAPER_API_KEY).
...
def run_your_gateway_project_setup(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for key_name in _OPENCLAW_KEY_NAMES:
        ...
        # Normalize: AUTH_TOKEN -> auth, SCRAPER_API_KEY -> scraper-vendor
...
# ---------------------------------------------------------------------------
# Device auth flow (GitHub OAuth via ScraperVendor)
# ---------------------------------------------------------------------------

_DEVICE_BASE = "https://api.your-scraper.example/v1/github/device"
...
def _existing_scraper_vendor_key() -> Optional[str]:
    ...
    try:
        ...
        if _env.CONFIG_FILE and _env.CONFIG_FILE.exists():
            return _env.load_env_file(_env.CONFIG_FILE).get("SCRAPER_API_KEY") or None
    except Exception as exc:  # never let a config-read failure block auth
        logger.debug("Could not read existing ScraperVendor key: %s", exc)
    return None
...
def run_device_auth() -> Optional[Tuple[str, str, str, int]]:
    ...
    POSTs to the ScraperVendor device/code endpoint.
...
def fetch_api_key(access_token: str) -> Optional[str]:
    """Fetch the Scraping API key using the GitHub access token.
    ...
    if not api_key:
        ...
        # for a GitHub account already linked to a ScraperVendor account. Log
...
def _start_device_flow() -> "Tuple[Dict[str, Any], Optional[Dict[str, Any]]]":
    ...
    if not _DEVICE_CODE_RE.match(user_code):
        ...
        return (
            {
                "status": "error",
                "message": "ScraperVendor returned an unexpected device-code format.",
            },
            None,
        )
