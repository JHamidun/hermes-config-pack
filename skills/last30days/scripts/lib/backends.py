...
- ``conditional`` (Reddit): routing is per-query and outcome-dependent —
  public keyless composite by default, ScraperVendor backfill only when
...
ScraperVendor) probe KEY PRESENCE ONLY: a dict lookup, never a network
...
# YouTube backend order (pipeline: yt-dlp first, ScraperVendor search
# fallback when yt-dlp is absent or fails — see lib/pipeline.py).
YOUTUBE_BACKEND_ORDER: Tuple[str, ...] = ("yt-dlp", "scraper-vendor")
...
_SC_PRESCRIPTION = (
    "set SCRAPER_API_KEY (free 10,000-call signup: "
    f"{prescriptions.get('scraper-vendor', 'key_missing').fix_cli})"
)
...
_SC_SPEC = BackendSpec(
    name="scraper-vendor",
    requires="SCRAPER_API_KEY",
    probe=_key_probe(
        "scraper-vendor", "SCRAPER_API_KEY", "SCRAPER_API_KEY",
...
def _resolve_conditional(
    ...
    has_key = bool(config.get("SCRAPER_API_KEY"))
    raw_pin = (config.get(descriptor.pin_var) or "").lower() if descriptor.pin_var else ""
    pinned_sc = has_key and raw_pin == "scraper-vendor"
    floor = _reddit_sc_min_items(config)

    if pinned_sc:
        res.pinned = True
        res.pin = "scraper-vendor"
        res.conditional = (
            f"ScraperVendor primary (pinned via {descriptor.pin_var}); "
    ...
    if has_key:
        if floor > 0:
            backfill = (
                f"ScraperVendor backfill when results fall below the "
                f"{floor}-item floor"
            )
        else:
            backfill = "ScraperVendor backfill when the free path returns nothing"
        res.conditional = f"public keyless composite (default); {backfill}"
        return res

    res.conditional = "public keyless composite (default); no ScraperVendor key for backfill"
    if raw_pin == "scraper-vendor":
        # The pipeline ignores the pin without a key; say so honestly.
        res.conditional += (
            f" ({descriptor.pin_var} pin ignored: SCRAPER_API_KEY not set)"
        )
    return res
