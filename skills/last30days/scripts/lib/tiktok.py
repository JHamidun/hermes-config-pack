"""TikTok search via Scraping API for /last30days.

Uses ScraperVendor REST API to search TikTok by keyword, extract engagement
...
SCRAPER_BASE = "https://api.your-scraper.example/v1/tiktok"
...
def _parse_date(item: Dict[str, Any]) -> Optional[str]:
    """Parse date from ScraperVendor TikTok item to YYYY-MM-DD."""
...
def _hashtag_search(
    ...
    """Search TikTok by hashtag via ScraperVendor.
...
def _profile_videos(
    ...
    """Fetch a TikTok creator's recent videos via ScraperVendor.
    ...
    profile_url = "https://api.your-scraper.example/v3/tiktok/profile/videos"
    try:
        data = http.get(
            profile_url,
...
def search_tiktok(
    ...
    """Search TikTok via Scraping API.
...
def fetch_captions(
    ...
    """Fetch transcripts for top N TikTok videos via ScraperVendor.
...
def enrich_with_comments(
    ...
    """Enrich top TikTok posts with comment data from ScraperVendor.
...
def _fetch_post_comments(
    ...
    """Fetch comments for a single TikTok post via ScraperVendor.
