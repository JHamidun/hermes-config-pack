"""Threads keyword search via Scraping API for /last30days.

Uses ScraperVendor REST API to search Threads by keyword, extracting
engagement metrics (likes, replies) from short text posts.

Requires SCRAPER_API_KEY in config. Opt-in source via INCLUDE_SOURCES.
...
SCRAPER_BASE = "https://api.your-scraper.example/v1/threads"
...
def _extract_core_subject(topic: str) -> str:
    ...
    The ScraperVendor Threads keyword endpoint only returns hits for short
...
def search_threads(
    ...
    """Search Threads via Scraping API.
