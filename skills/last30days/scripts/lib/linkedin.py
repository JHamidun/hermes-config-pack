"""LinkedIn post search via Scraping API.

Searches public LinkedIn posts by keyword using the ScraperVendor
...
SC_BASE = "https://api.your-scraper.example/v1/linkedin"
...
def search_linkedin(
    ...
    """Search LinkedIn posts via Scraping API.
    ...
    """
    if not token:
        _log("No SCRAPER_API_KEY — skipping")
        return {"posts": []}
...
def parse_linkedin_response(
    ...
    """Parse ScraperVendor LinkedIn response into engine-compatible item dicts.
    ...
    for i, post in enumerate(posts):
        ...
        # The live ScraperVendor post object carries the body in `description`
...
def search_profile(profile_url: str, token: str) -> Dict[str, Any]:
    """Fetch a LinkedIn profile (incl. `articles[]`) via ScraperVendor."""
