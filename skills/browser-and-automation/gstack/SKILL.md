---
name: gstack
description: "Быстрый невидимый браузер для проверки сайта."
user_description: "Быстрый невидимый браузер для проверки сайта: открывает адрес, выполняет действия на странице, показывает разницу состояний до и после, снимает скриншоты — примерно за десятую долю секунды на команду. Нужен, когда после выкатки надо за минуту убедиться, что страница жива и сценарий проходится."
version: 1.1.0
license: MIT
metadata:
  hermes:
    category: browser-and-automation
    tags: [gstack, node, git, pdf, claude]
    source: claude-code-config-pack
    requires_tools: [-, clarify, read_file, terminal]
---
## Когда применять

Быстрый headless-браузер (~100 мс) для QA: переходы, действия, дифф, скриншоты. Триггеры: «быстрый браузер», «gstack browse», «проверь деплой».
...
[ -z "$B" ] && B=~/.hermes/skills/browser-and-automation/gstack/browse/dist/browse
if [ -x "$B" ]; then
  echo "READY: $B"
else
  echo "NEEDS_SETUP"
...
3. Run: `cd "~/.hermes/skills/browser-and-automation/gstack" && bash ./setup`
...
- NEVER use `mcp_claude_in_chrome_*` tools. They are slow and unreliable.
