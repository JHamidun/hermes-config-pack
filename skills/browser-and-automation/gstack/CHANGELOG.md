...
- Merged ELI16 rules into base clarify format — one format instead of two, no `_SESSIONS >= 3` conditional.
...
- Upgrade skill template bumped to v1.1.0 with `write_file` tool permission for config editing.
...
- Stale servers from global install no longer shadow local changes — removed legacy `~/.hermes/skills/browser-and-automation/gstack` fallback from `resolveServerScript()`
...
- Legacy fallback to `~/.hermes/skills/browser-and-automation/gstack/browse/src/server.ts`
