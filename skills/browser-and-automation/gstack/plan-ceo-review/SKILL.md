...
description: "CEO/founder-mode plan review. Rethink the problem."
version: 1.0.0
license: MIT
metadata:
  hermes:
    category: browser-and-automation
    tags: [gstack, plan, ceo, review, node, git, sql, claude]
...
**STOP.** clarify once per issue. Do NOT batch. Recommend + WHY. If no issues or fix is obvious, state what you'll do and move on — don't waste a question. Do NOT proceed until user responds.
...
### TODOS.md updates
Present each potential TODO as its own individual clarify. Never batch TODOs — one per question. Never silently skip this step. Follow the format in `.claude/skills/review/TODOS-format.md`.
...
Identify at least 5 "bonus chunk" opportunities (<30 min each) that would make users think "oh nice, they thought of that." Present each delight opportunity as its own individual clarify. Never batch them. For each one, describe what it is, why it would delight users, and effort estimate. Then present options: **A)** Add to TODOS.md as a vision item **B)** Skip **C)** Build it now in this PR.
...
If any clarify goes unanswered, note it here. Never silently default.
