...
Report the exact output. Do NOT try to fix or install anything — just report what you see.`,
    ...
    // ~/.hermes/skills/browser-and-automation/gstack/browse/dist/browse exists, so we get READY.
...
File a contributor report about this issue. Then tell me what you filed.`,
  ...
  test('session awareness adds ELI16 context when _SESSIONS >= 3', async () => {
    ...
    // Extract clarify format instructions from generated SKILL.md
    const skillMd = fs.readFileSync(path.join(ROOT, 'SKILL.md'), 'utf-8');
    const aqStart = skillMd.indexOf('## clarify Format');
...
Since this is non-interactive, DO NOT actually call clarify. Instead, write the EXACT text you would display to the user (the full clarify content) to the file: ${outputPath}
...
Do NOT use clarify — run Quick tier directly.
...
Choose HOLD SCOPE mode. Skip any clarify calls — this is non-interactive.
...
Choose SMALL CHANGE mode. Skip any clarify calls — this is non-interactive.
Write your complete review directly to ${planDir}/review-output.md
...
Run /retro for the last 7 days of this git repo. Skip any clarify calls — this is non-interactive.
...
describeE2E('QA-Only skill E2E', () => {
  ...
  test('/qa-only produces report without using patch tool', async () => {
    const result = await runSkillTest({
...
Write your report to ${qaOnlyDir}/qa-reports/qa-only-report.md`,
    ...
    if (editCalls.length > 0) {
      console.warn('qa-only used patch tool:', editCalls.length, 'times');
    }
...
This is a test+fix loop: find bugs, fix them in the source code, commit each fix, and re-verify.`,
    ...
    // Verify patch tool was used (agent actually modified source code)
    const editCalls = result.toolCalls.filter(tc => tc.tool === 'Edit');
...
IMPORTANT:
- Do NOT use clarify — auto-approve everything or skip if unsure.
