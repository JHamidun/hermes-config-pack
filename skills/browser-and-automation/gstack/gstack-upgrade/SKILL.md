---
name: gstack-upgrade
description: "Upgrade gstack to the latest version. Detects global vs."
version: 1.1.0
license: MIT
metadata:
  hermes:
    category: browser-and-automation
    tags: [gstack, upgrade, git, claude]
...
[ -z "$_AUTO" ] && _AUTO=$(bash ~/.hermes/skills/browser-and-automation/gstack/bin/gstack-config get auto_upgrade 2>/dev/null || true)
echo "AUTO_UPGRADE=$_AUTO"
```

**If `AUTO_UPGRADE=true` or `AUTO_UPGRADE=1`:** Skip clarify. Log "Auto-upgrading gstack v{old} → v{new}..." and proceed directly to Step 2. If `./setup` fails during auto-upgrade, restore from backup (`.bak` directory) and warn the user: "Auto-upgrade failed — restored previous version. Run `/gstack-upgrade` manually to retry."
...
```bash
bash ~/.hermes/skills/browser-and-automation/gstack/bin/gstack-config set auto_upgrade true
```
...
**If "Never ask again":**
```bash
bash ~/.hermes/skills/browser-and-automation/gstack/bin/gstack-config set update_check false
```
Tell user: "Update checks disabled. Run `bash ~/.hermes/skills/browser-and-automation/gstack/bin/gstack-config set update_check true` to re-enable."
Continue with the current skill.

### Step 2: Detect install type

```bash
if [ -d "~/.hermes/skills/browser-and-automation/gstack/.git" ]; then
  INSTALL_TYPE="global-git"
  INSTALL_DIR="~/.hermes/skills/browser-and-automation/gstack"
  ...
  INSTALL_DIR=".claude/skills/gstack"
elif [ -d "~/.hermes/skills/browser-and-automation/gstack" ]; then
...
```bash
bash ~/.hermes/skills/browser-and-automation/gstack/bin/gstack-update-check --force
```
