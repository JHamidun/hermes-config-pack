...
description: "Post-ship documentation update. Reads all project docs."
version: 1.0.0
license: MIT
metadata:
  hermes:
    category: browser-and-automation
    tags: [gstack, document, release, node, git, claude]
...
- Bump VERSION without asking — always use clarify for version changes
- Use `write_file` tool on CHANGELOG.md — always use `patch` with exact `old_string` matches
...
For each risky or questionable update identified in Step 2, use clarify with:
...
3. Never regenerate a CHANGELOG entry from scratch. The entry was written by `/ship` from the
   ...
   rewriting history.
4. If an entry looks wrong or incomplete, use clarify — do NOT silently fix it.
5. Use patch tool with exact `old_string` matches — never use Write to overwrite CHANGELOG.md.
...
- Auto-fix minor voice adjustments. Use clarify if a rewrite would alter meaning.
...
5. Flag any contradictions between documents. Auto-fix clear factual inconsistencies (e.g., a
   version mismatch). Use clarify for narrative contradictions.
...
2. **Items needing description updates:** If a TODO references files or components that were
   significantly changed, its description may be stale. Use clarify to confirm whether
   the TODO should be updated, completed, or left as-is.

3. **New deferred work:** Check the diff for `TODO`, `FIXME`, `HACK`, and `XXX` comments. For
   each one that represents meaningful deferred work (not a trivial inline note), use
   clarify to ask whether it should be captured in TODOS.md.
...
3. **If VERSION was NOT bumped:** Use clarify:
...
4. **If VERSION was already bumped:** Do NOT skip silently. Instead, check whether the bump
   ...
   d. **If there are significant uncovered changes:** Use clarify explaining what the
