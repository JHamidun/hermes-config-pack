---
name: seo-machine-ru
description: "Ведёт SEO-контент от запроса до публикации."
user_description: "Ведёт SEO-контент от запроса до публикации: собирает семантику по Wordstat, планирует кластер статей, пишет лонгрид, оценивает его по чеклисту качества и выкладывает на сайт. Нужен, когда поисковый трафик должен идти системно, а не отдельными удачными статьями."
version: 1.0.0
license: MIT
metadata:
  hermes:
    category: marketing
    tags: [seo, machine, playwright, docker, python, docx]
    source: claude-code-config-pack
---
## Когда применять

SEO/GEO/AEO контент-машина: лонгриды под Яндекс (Wordstat) и Tilda, семантическое ядро. Триггеры: «напиши SEO-статью», «попасть в AI-ответы».
...
**Перед любой задачей** прочитай `~/.hermes/ccpack/business-context.md` — разделы «Продукт», «ICP», «Позиционирование и запреты». Пишешь для личного блога, а не для продукта — читай вместо этого `~/.hermes/ccpack/author-profile.md`. Обоих файлов нет — заведи из `~/.hermes/ccpack/templates/`.
...
- Бренд-голос и конкуренты — НЕ дублировать: брать из `~/.hermes/ccpack/business-context.md`, `~/.hermes/ccpack/voice-sample.md` и `competitive-analysis`.
...
- **Wordstat:** Direct API заблокирован (`error 58` — нужен одобренный доступ к Директ API). Реальный способ — internal API `POST /wordstat/api/getTable` с залогиненного браузера (куки сами, CSRF не нужен). Креды Яндекса — свои, в `$HERMES_HOME/.env` (`YANDEX_EMAIL`/`YANDEX_PASSWORD`); 2FA проходишь руками один раз, дальше сессия живёт в профиле playwright/chrome-devtools.
