...
description: "Готовит сообщение под конкретного получателя."
user_description: "Готовит сообщение под конкретного получателя: руководителю, заказчику, команде — с нужным уровнем детализации. Нужен, когда одно и то же надо рассказать по-разному."
user_description_i18n:
  ar: "يُعدّ رسالة موجّهة إلى متلقٍّ محدد: مدير أو عميل أو فريق، بمستوى التفصيل المناسب لكل منهم. مفيد عندما يجب سرد الأمر نفسه بطرق مختلفة لأشخاص مختلفين."
  en: "Prepares a message for a specific recipient: an executive, a client, or the team, with the right level of detail for each. Useful when the same news has to be told differently to different people."
  es: "Prepara un mensaje para un destinatario concreto: un directivo, un cliente o el equipo, con el nivel de detalle que cada uno necesita. Útil cuando lo mismo hay que contarlo de forma distinta a cada persona."
  fr: "Prépare un message pour un destinataire précis : un dirigeant, un client ou l'équipe, avec le niveau de détail adapté à chacun. Utile quand la même chose doit être racontée différemment selon la personne."
  ja: "相手に合わせてメッセージを用意します。経営層、顧客、チームそれぞれに適した詳しさで。同じ内容を相手ごとに違う伝え方で話す必要があるときに役立ちます。"
  pt: "Prepara uma mensagem para um destinatário específico: um executivo, um cliente ou a equipe, com o nível de detalhe certo para cada um. Útil quando a mesma coisa precisa ser contada de jeitos diferentes para pessoas diferentes."
  zh: "针对特定收件人来撰写消息：给上级、给客户、给团队，各自采用合适的详略程度。适合同一件事需要向不同的人用不同方式讲述的时候。"
  zh-hant: "針對特定收件人來撰寫訊息：給主管、給客戶、給團隊，各自採用合適的詳略程度。適合同一件事需要向不同的人用不同方式說明的時候。"
version: 1.0.0
license: MIT
metadata:
  hermes:
    category: writing-and-communication
    tags: [stakeholder, comms]
    source: claude-code-config-pack
---
## Когда применять

Апдейты для стейкхолдеров под аудиторию: статусы, отчёты, анонсы, риски. Триггеры: «апдейт для руководства», «недельный статус», «письмо про риск». НЕ рассылки/FAQ→internal-comms.
