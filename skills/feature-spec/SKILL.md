---
name: feature-spec
description: "Пишет спецификацию фичи: зачем она, для кого."
user_description: "Пишет спецификацию фичи: зачем она, для кого, что считается готовым, как поймём, что сработало. Нужен перед разработкой, чтобы не переделывать."
user_description_i18n:
  ar: "يكتب مواصفات الميزة: لماذا نحتاجها، ولمن هي، وما الذي يُعدّ منجزًا، وكيف نعرف أنها نجحت. تحتاجه قبل بدء التطوير حتى لا تضطر إلى إعادة العمل لاحقًا."
  en: "Writes a feature specification: why it exists, who it is for, what counts as done and how you will know it worked. Use it before development starts so nothing has to be rebuilt later."
  es: "Redacta la especificación de una funcionalidad: para qué sirve, para quién es, qué se considera terminado y cómo sabremos que funcionó. Útil antes de empezar el desarrollo, para no tener que rehacer nada."
  fr: "Rédige la spécification d'une fonctionnalité : pourquoi elle existe, pour qui, ce qui compte comme terminé et comment savoir qu'elle a fonctionné. Utile avant le développement, pour ne pas avoir à tout refaire."
  ja: "機能の仕様書を作成します。なぜ必要か、誰のためか、何をもって完成とするか、成功をどう判断するかをまとめます。作り直しを避けるため、開発を始める前に使います。"
  pt: "Escreve a especificação de uma funcionalidade: por que ela existe, para quem é, o que conta como pronto e como saber que funcionou. Útil antes de começar o desenvolvimento, para não ter que refazer nada."
  zh: "撰写功能规格说明：为什么要做、面向谁、什么算完成、如何判断它奏效。在开发开始前使用，避免日后返工。"
  zh-hant: "撰寫功能規格說明：為什麼要做、面向誰、什麼算完成、如何判斷它奏效。在開發開始前使用，避免日後返工。"
version: 1.0.0
license: MIT
metadata:
  hermes:
...
PRD и спеки фич: user stories, требования, критерии приёмки, метрики. Триггеры: «PRD», «напиши спеку». НЕ: RFC→doc-coauthoring; роадмап→roadmap-management.
