...
description: "Возвращает проект в состояние до неудачного шага."
user_description: "Возвращает проект в состояние до неудачного шага: восстанавливает файлы из резервных копий по журналу изменений, удаляет созданное, откатывает коммиты и подчищает временные артефакты. Нужен, когда автоматическая правка пошла не так и надо вернуть всё как было, а не разбирать последствия вручную."
version: 1.0.0
license: MIT
metadata:
  hermes:
    category: git-and-workflow
    tags: [rollback, changes, git]
    source: claude-code-config-pack
    requires_tools: [read_file, terminal, write_file]
---
## Когда применять

Откат упавшей фазы воркфлоу по changes-log: восстановление файлов, чистка артефактов, отмена команд. Триггеры: «откати изменения», «верни как было».
...
Use read_file tool to load JSON. Validate required fields: phase, timestamp, files_modified, files_created, commands_executed, git_commits.
