---
name: beads
description: "Ведёт задачи и баги проекта прямо в репозитории."
user_description: "Ведёт задачи и баги проекта прямо в репозитории: заводит задачи, связывает зависимости между ними, показывает, что готово к работе прямо сейчас, и синхронизирует всё через git. Нужен, когда работа тянется через много сессий и надо не терять нить, не заводя отдельный трекер."
version: 1.0.0
license: MIT
metadata:
  hermes:
...
Beads — git-трекер задач и багов для AI-агентов (команды bd). Триггеры: «заведи задачу в beads», «issue tracking», «список задач репо».
