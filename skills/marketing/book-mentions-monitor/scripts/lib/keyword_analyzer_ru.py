...
Если установлен pymorphy3 — лемматизируем; иначе грубая нормализация
...
# Лемматизатор. Ставится pymorphy3 (requirements.txt); pymorphy2 годится только для
# Python < 3.11 — на 3.11+ он импортируется, но падает при создании MorphAnalyzer:
# AttributeError: module 'inspect' has no attribute 'getargspec'.
#
# Провал здесь НЕ обязан быть тихим. Без лемматизатора «книга», «книги» и
# «книгой» — три разных слова: упоминания недосчитываются, дедупликация (dedup.py)
# и обогащение (enrich.py) молча разъезжаются вместе с ним.
_MORPH = None
...
FALLBACK_WARNING = (
    ...
    "  Словоформы одного слова («книга», «книги», «книгой») считаются разными словами.\n"
    f"  Причина: {_MORPH_ERROR or 'pymorphy3 не установлен'}\n"
    "  Починить: pip install pymorphy3"
)

if _MORPH is None:
    # Модуль импортируют как библиотеку (dedup.py, enrich.py, report_xlsx.py), поэтому
    # предупреждение печатается при импорте, а не только в CLI.
    print("\n[!] " + FALLBACK_WARNING + "\n", file=sys.stderr)
