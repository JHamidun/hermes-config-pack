#!/usr/bin/env python
"""
llms.txt Generator — валидация и генерация llms.txt / llms-full.txt.

llms.txt — emerging-стандарт навигации для AI-краулеров: /llms.txt в корне домена
(краткая карта сайта в markdown), /llms-full.txt — расширенная версия.
...
- RU-пути в категоризации страниц (/uslugi, /tseny, /stati, /novosti, /o-kompanii, /kontakty).

Usage:
    python llmstxt_generator.py <url>            # validate (default)
    python llmstxt_generator.py <url> generate   # crawl homepage → сгенерировать llms.txt
"""
...
def validate_llmstxt(url: str) -> dict:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if "contact" not in content.lower() and "контакт" not in content.lower():
...
def generate_llmstxt(url: str, max_pages: int = 30) -> dict:
    """Generate an llms.txt file by crawling the homepage."""
    parsed = urlparse(url)
    base_url = f"{parsed.scheme}://{parsed.netloc}"
    ...
    # llms-full.txt (with per-page descriptions)
        ...
        if section_pages:
            ...
            for page in section_pages:
                ...
                try:
                    ...
                    page_soup = BeautifulSoup(page_resp.text, PARSER)
