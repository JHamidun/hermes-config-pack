...
4. **Signal to user and return control**:
   ...
   2. Invoke bug-hunter via delegate_task tool
...
**IMPORTANT**: Claude Code does NOT have automatic agent invocation. The main session must explicitly invoke workers using the delegate_task tool.
...
4. **Main session** explicitly invokes worker using delegate_task tool:
   ```
   Use delegate_task tool with:
...
2. **Track progress** with todo (mark phases in_progress → completed immediately)
...
**Solution**: Remove delegate_task tool usage, use Return Control pattern
