...
| Communication | Direct delegate_task tool delegation | Plan files |
| Context | Full conversation history | Isolated agent context |
| Tool access | Main session uses delegate_task tool freely | Orchestrator agents cannot use delegate_task tool |
...
Slash Command (main orchestrator)
  ↓
delegate_task tool → bug-orchestrator (suborchestrator)
  ↓
...
Command reads plan file
  ↓
delegate_task tool → bug-hunter (worker)
  ↓
Worker executes, generates report, returns control
  ↓
Command → delegate_task tool → bug-orchestrator (validation)
  ↓
...
- Invoke suborchestrators via delegate_task tool
- Read plan files between phases
- Invoke workers via delegate_task tool
- Display results to user

**Responsibilities**:
- Phase coordination
- Read plan files to determine next steps
- Invoke agents explicitly with delegate_task tool
...
**Critical Rules**:
- NO delegate_task tool usage (use plan files instead)
...
**Why no delegate_task tool?**
Orchestrator agents run in isolated context without access to delegate_task tool. They coordinate through file-based communication (plan files).
...
2. Agent uses available tools (Grep, Read, web_search, Context7, Supabase docs)
...
**Why needed?**
Claude Code does NOT support automatic agent invocation. Main session must explicitly invoke all agents using delegate_task tool.

**Pattern**:
1. Orchestrator creates plan file with `nextAgent` field
2. Orchestrator reports readiness and exits
3. Main session (command) reads plan file
4. Main session invokes worker using delegate_task tool with `subagent_type` from plan
...
- Track progress with todo (in_progress → completed immediately)

**For Agent Orchestrators (Health Workflows)**:
- Always validate plan files after creation using `validate-plan-file` Skill
- Track progress with todo (mark phases in_progress → completed immediately)
...
**For Commands**:
- Read plan files to determine next steps
- Explicitly invoke agents using delegate_task tool
...
```
delegate_task tool → meta-agent-v3
...
```
delegate_task tool → fullstack-nextjs-specialist
Prompt: "Implement these 5 parallel tasks: T001, T002, T003, T004, T005"
```

✅ **CORRECT** (1 task = 1 agent run, all in parallel):
```
# Launch all 5 simultaneously in single message with 5 delegate_task tool calls
delegate_task tool → fullstack-nextjs-specialist
...
```
1. delegate_task tool → database-architect
   Prompt: "Implement T001: Create database schema"
   Wait for completion, verify results

2. delegate_task tool → fullstack-nextjs-specialist
   Prompt: "Implement T002: Create User model (depends on T001)"
   Wait for completion, verify results

3. delegate_task tool → fullstack-nextjs-specialist
...
**Orchestrator using delegate_task tool (Health Workflows)**:
- Problem: Orchestrator tries to invoke subagents directly
- Solution: Remove delegate_task tool usage, create plan files, return control
