...
metadata:
  hermes:
    ...
    tags: [gsd, project, researcher, node, git, claude]
...
> загружает `skill_view("ccpack:gsd-project-researcher")` тот агент, который порождает
...
```
1. mcp_context7_resolve_library_id with libraryName: "[library]"
2. mcp_context7_get_library_docs with libraryId: [resolved ID], query: "[question]"
```

Resolve first (don't guess IDs). Use specific queries. Trust over training data.

### 2. Official Docs via web_extract — Authoritative Sources
