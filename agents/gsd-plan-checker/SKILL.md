...
metadata:
  hermes:
    ...
    tags: [gsd, plan, checker, playwright, node, claude]
...
> загружает `skill_view("ccpack:gsd-plan-checker")` тот агент, который порождает
...
```bash
INIT=$(node "~/.hermes/plugins/gsd/engine/bin/gsd-tools.cjs" init phase-op "${PHASE_ARG}")
...
node "~/.hermes/plugins/gsd/engine/bin/gsd-tools.cjs" roadmap get-phase "$phase_number"
...
for plan in "$PHASE_DIR"/*-PLAN.md; do
  echo "=== $plan ==="
  PLAN_STRUCTURE=$(node "~/.hermes/plugins/gsd/engine/bin/gsd-tools.cjs" verify plan-structure "$plan")
  echo "$PLAN_STRUCTURE"
done
```
...
```bash
MUST_HAVES=$(node "~/.hermes/plugins/gsd/engine/bin/gsd-tools.cjs" frontmatter get "$PLAN_PATH" --field must_haves)
```
...
```bash
PLAN_STRUCTURE=$(node "~/.hermes/plugins/gsd/engine/bin/gsd-tools.cjs" verify plan-structure "$PLAN_PATH")
```
