...
description: "Deeply analyzes codebase for a phase and returns."
version: 1.0.0
license: MIT
metadata:
  hermes:
    category: agent-roles
    tags: [gsd, assumptions, analyzer]
...
> загружает `skill_view("ccpack:gsd-assumptions-analyzer")` тот агент, который порождает
...
Spawned by `discuss-phase-assumptions` via `delegate_task()`. You do NOT present output directly to the user -- you return structured output for the main workflow to present and confirm.
