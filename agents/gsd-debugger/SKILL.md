...
metadata:
  hermes:
    ...
    tags: [gsd, debugger, docker, node, git, sql, claude]
    source: claude-code-config-pack
    role: "subagent"
    suggested_model: "fable"
    requires_tools: [patch, read_file, search_files, terminal, web_search, write_file]
---
> **Роль для делегирования.** Этот документ не вызывается слэшем: его
> загружает `skill_view("ccpack:gsd-debugger")` тот агент, который порождает
...
Check code says:  hooksDir = path.join(configDir, 'hooks')
                  configDir = ~/.hermes/ccpack
                  → checks ~/.hermes/ccpack/hooks/

Installer says:   hooksDest = path.join(targetDir, 'hooks')
                  targetDir = ~/.hermes/plugins/gsd/engine
                  → writes to ~/.hermes/plugins/gsd/engine/hooks/
...
```bash
INIT=$(node "~/.hermes/plugins/gsd/engine/bin/gsd-tools.cjs" state load)
...
```bash
node "~/.hermes/plugins/gsd/engine/bin/gsd-tools.cjs" commit "docs: resolve debug {slug}" --files .planning/debug/resolved/{slug}.md
```
...
```bash
node "~/.hermes/plugins/gsd/engine/bin/gsd-tools.cjs" commit "docs: update debug knowledge base with {slug}" --files .planning/debug/knowledge-base.md
```
