...
description: "Synthesizes research outputs from parallel researcher."
version: 1.0.0
license: MIT
metadata:
  hermes:
    category: agent-roles
    tags: [gsd, research, synthesizer, node, git]
...
> загружает `skill_view("ccpack:gsd-research-synthesizer")` тот агент, который порождает
...
Use template: ~/.hermes/plugins/gsd/engine/templates/research-project/SUMMARY.md
...
```bash
node "~/.hermes/plugins/gsd/engine/bin/gsd-tools.cjs" commit "docs: complete project research" --files .planning/research/
```
