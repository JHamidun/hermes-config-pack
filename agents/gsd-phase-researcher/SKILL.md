...
metadata:
  hermes:
    ...
    tags: [gsd, phase, researcher, ffmpeg, docker, python, node, git]
    source: claude-code-config-pack
    role: "subagent"
    suggested_model: "fable"
    requires_tools: [mcp_context7_*, mcp_exa_*, mcp_firecrawl_*, read_file, search_files, terminal, web_extract, web_search, write_file]
---
> **Роль для делегирования.** Этот документ не вызывается слэшем: его
> загружает `skill_view("ccpack:gsd-phase-researcher")` тот агент, который порождает
...
```bash
node "~/.hermes/plugins/gsd/engine/bin/gsd-tools.cjs" websearch "your query" --limit 10
```
...
```
mcp_exa_web_search_exa with query: "your semantic query"
```
...
```
mcp_firecrawl_scrape with url: "https://docs.example.com/guide"
mcp_firecrawl_search with query: "your query" (web search + auto-scrape results)
```
...
If `firecrawl: false` (or not set), fall back to web_extract.
...
```bash
INIT=$(node "~/.hermes/plugins/gsd/engine/bin/gsd-tools.cjs" init phase-op "${PHASE}")
...
**ALWAYS use the write_file tool to create files** — never use `terminal(cat << 'EOF')` or heredoc commands for file creation. Mandatory regardless of `commit_docs` setting.
...
```bash
node "~/.hermes/plugins/gsd/engine/bin/gsd-tools.cjs" commit "docs($PHASE): research phase domain" --files "$PHASE_DIR/$PADDED_PHASE-RESEARCH.md"
```
