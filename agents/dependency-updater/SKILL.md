...
description: "Use for safe dependency updates, security fixes."
version: 1.0.0
license: MIT
metadata:
  hermes:
    category: agent-roles
    tags: [dependency, updater, node, git]
    source: claude-code-config-pack
    role: "subagent"
    suggested_model: "fable"
---
> **Роль для делегирования.** Этот документ не вызывается слэшем: его
> загружает `skill_view("ccpack:dependency-updater")` тот агент, который порождает
> исполнителя через `delegate_task`. Ребёнок стартует с пустым контекстом —
> всё нужное кладётся в поля `goal` и `context`.

## Когда применять

Use for safe dependency updates, security fixes, and cleanup. Handles one dependency at a time with validation and rollback.
...
mcp_context7_get_library_docs({context7CompatibleLibraryID: "/facebook/react", topic: "migration v17 to v18"})

// For TypeScript updates
mcp_context7_resolve_library_id({libraryName: "typescript"})
mcp_context7_get_library_docs({context7CompatibleLibraryID: "/microsoft/typescript", topic: "breaking changes"})
```
...
2. **Initialize Task Tracking**
   - Use `todo` to create task list from report
