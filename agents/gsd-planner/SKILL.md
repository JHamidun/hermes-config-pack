...
metadata:
  hermes:
    ...
    tags: [gsd, planner, node, git, openai, claude, audio]
    source: claude-code-config-pack
    role: "subagent"
    suggested_model: "fable"
    requires_tools: [mcp_context7_*, read_file, search_files, terminal, web_extract, write_file]
---
> **Роль для делегирования.** Этот документ не вызывается слэшем: его
> загружает `skill_view("ccpack:gsd-planner")` тот агент, который порождает
...
<execution_context>
@~/.hermes/plugins/gsd/engine/workflows/execute-plan.md
@~/.hermes/plugins/gsd/engine/templates/summary.md
</execution_context>
...
```bash
node "~/.hermes/plugins/gsd/engine/bin/gsd-tools.cjs" commit "fix($PHASE): revise plans based on checker feedback" --files .planning/phases/$PHASE-*/$PHASE-*-PLAN.md
```
...
```bash
INIT=$(node "~/.hermes/plugins/gsd/engine/bin/gsd-tools.cjs" init plan-phase "${PHASE}")
...
```bash
node "~/.hermes/plugins/gsd/engine/bin/gsd-tools.cjs" history-digest
```
...
```bash
VALID=$(node "~/.hermes/plugins/gsd/engine/bin/gsd-tools.cjs" frontmatter validate "$PLAN_PATH" --schema plan)
```
...
```bash
STRUCTURE=$(node "~/.hermes/plugins/gsd/engine/bin/gsd-tools.cjs" verify plan-structure "$PLAN_PATH")
```
...
```bash
node "~/.hermes/plugins/gsd/engine/bin/gsd-tools.cjs" commit "docs($PHASE): create phase plan" --files .planning/phases/$PHASE-*/$PHASE-*-PLAN.md .planning/ROADMAP.md
```
</step>
