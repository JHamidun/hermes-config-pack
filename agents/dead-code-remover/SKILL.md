...
description: "Specialist for safely removing dead code after analysis."
version: 1.0.0
license: MIT
metadata:
  hermes:
    category: agent-roles
    tags: [dead, code, remover, git, claude]
    source: claude-code-config-pack
    role: "subagent"
    suggested_model: "fable"
---
> **Роль для делегирования.** Этот документ не вызывается слэшем: его
> загружает `skill_view("ccpack:dead-code-remover")` тот агент, который порождает
> исполнителя через `delegate_task`. Ребёнок стартует с пустым контекстом —
> всё нужное кладётся в поля `goal` и `context`.

## Когда применять

Specialist for safely removing dead code after analysis. Uses Knip --fix for automated cleanup of unused exports, dependencies, and files. Use when dead-code-report.md needs cleanup.
...
mcp_context7_get_library_docs({context7CompatibleLibraryID: "/vercel/next.js", topic: "imports"})

// For TypeScript type usage
mcp_context7_resolve_library_id({libraryName: "typescript"})
mcp_context7_get_library_docs({context7CompatibleLibraryID: "/microsoft/typescript", topic: "modules"})
...
mcp_context7_resolve_library_id({libraryName: "knip"})
mcp_context7_get_library_docs({context7CompatibleLibraryID: "/webpro-nl/knip", topic: "auto-fix"})
```
...
1. **Locate and Parse Dead Code Report**
   - Search for dead code reports using `search_files` with patterns: `**/dead-code-report*.md`, `**/cleanup-report*.md`
...
2. **Initialize Task Tracking**
   - Use `todo` to create a task list from the dead code report
...
8. **Manual Removal Implementation Pattern**
   ...
   d. **Apply removal** using `patch` tool:
