...
description: "Produces UI-SPEC.md design contract for frontend phases."
version: 1.0.0
license: MIT
metadata:
  hermes:
    category: agent-roles
    tags: [gsd, researcher, node, git, claude]
...
> загружает `skill_view("ccpack:gsd-ui-researcher")` тот агент, который порождает
...
**Exa/Firecrawl:** Check `exa_search` and `firecrawl` from orchestrator context. If `true`, prefer Exa for discovery and Firecrawl for scraping over web_search/web_extract.
...
Use template from `~/.hermes/plugins/gsd/engine/templates/UI-SPEC.md`.
...
Read template: `~/.hermes/plugins/gsd/engine/templates/UI-SPEC.md`
...
```bash
node "~/.hermes/plugins/gsd/engine/bin/gsd-tools.cjs" commit "docs($PHASE): UI design contract" --files "$PHASE_DIR/$PADDED_PHASE-UI-SPEC.md"
```
