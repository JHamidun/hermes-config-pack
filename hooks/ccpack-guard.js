...
try {
  ...
  if (hit) {
    try {
      const logDir = path.join(HOMEwin, '.hermes', 'ccpack', 'hooks-logs');
    ...
    process.stderr.write('BLOCKED by ccpack guard [' + hit.id + ']: ' + hit.why +
...
try {
  ...
  // Hermes names the same tools differently. Mapping here keeps both guard
  // bodies below identical to the Claude Code original.
  const HERMES_TOOLS = { terminal: 'Bash', process: 'Bash', shell: 'Bash',
                         write_file: 'Write', patch: 'Edit', edit_file: 'Edit' };
  const rawTool = (data && data.tool_name) || '';
  const tool = HERMES_TOOLS[rawTool] || rawTool;
